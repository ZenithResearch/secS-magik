...
| `server/src/evidence.rs` | Evidence adapter seam. | Defines typed evidence requests/results, deterministic `local_static` local-dev-test adapter, and cryptographic `wallet_presentation` proof-of-possession for the claimed subject using the explicitly temporary minimal-equivalent secS challenge contract; full Castalia Wallet wallet-core import/parity, receiver trust/root/registry authority, plus Dregg, Midnight, and Cardano remain future/optional rails. |
