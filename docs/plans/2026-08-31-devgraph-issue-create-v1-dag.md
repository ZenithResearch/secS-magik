...
| DG-E | Partial on this branch: DG-E1 fixed producer adapter only | Cross-repository evidence: prove one create, exact duplicate, denial matrix, receipt correlation, and no credential/log leakage. | DG-E1 proves only safe local projection production. Full reproducible cross-repository evidence remains separate. |
...
DG-E1 on this branch adds one fixed owner-private local adapter around DG-P.
This secS repository still does not claim Devgraph Work mutation, Wallet live
custody, transport/deployment, hybrid/PQ authorization, or cross-repository
