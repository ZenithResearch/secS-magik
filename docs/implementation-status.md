...
| Node/verifier identity and public-key seam | `server/src/identity.rs`, `server/tests/identity.rs` | Solid / implemented for explicit local file config plus B2 local public-key lookup seam | `production_verified` requires an explicit verifier key path before signing; missing, inaccessible, and malformed files fail as typed identity config errors; default signer ids are deterministic Ed25519 public-key fingerprints; unsafe override ids that look like paths or secret material reject; contexts/receipts can be verified through a receiver-held `PublicVerifierKeyRegistry` that fails unknown ids and wrong-key signatures; production gateway startup loads this identity for signed contexts and verify/execute receipts, while local/dev router receipts are stamped `local_dev_untrusted`. |
...
| Identity/signature helpers for contexts/receipts | `server/src/identity.rs` | Implemented through B3: production identity loading requires an operator-provided key path, default `signer_key_id` is derived as `ed25519:<sha256-public-key-fingerprint>`, safe explicit overrides reject path/secret-shaped ids, contexts/receipts sign with the configured identity, the gateway uses the loaded identity for production-shaped signed contexts and verify/execute receipts, local/dev receipts remain stamped `local_dev_untrusted`, and `PublicVerifierKeyRegistry` provides the first local public-key lookup seam for unknown-id and wrong-key rejection. |
...
- “configured signed context/receipt posture through B3” for current Ed25519 contexts/receipts, while still caveating that rotation/revocation and issuer/root trust policy are not complete;
