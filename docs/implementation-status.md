...
| Prototype gateway/router | `server/src/gateway.rs` | Partial / prototype | Preserves legacy `node_telemetry`, persists verify/execution receipts and handler lifecycle events in the ledger, carries an explicit expected receiver audience from runtime config, and routes configured machine programs from signed verified contexts through bounded payload-size/timeout checks. It is not yet a durable/general execution broker. |
