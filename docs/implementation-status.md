...
| Prototype ingress | `server/src/ingress.rs` | Partial / prototype with Track G read hardening | Deserializes packets only after bounded wire reads, calls the prototype verifier/payload path, decrypts payloads by runtime mode, looks up manifest descriptors, signs a `VerifiedCallContext`, emits reject receipts for typed verifier/payload/manifest failures, and hands verified payloads to the gateway router. The read/memory boundary is hardened, but connection/task concurrency remains outside this Track G claim. |
| Prototype gateway/router / bounded execution broker | `server/src/gateway.rs` | Solid / implemented as receiver-local bounded handler routing | Preserves legacy `node_telemetry`, persists verify/execution receipts and handler lifecycle events in the ledger, carries an explicit expected receiver audience from runtime config, selects handlers by descriptor `handler_id` from signed verified contexts, enforces payload/output size and timeout limits, emits execution receipts for success/failure/unavailable/timeout/oversized outcomes, and gates dev subprocess handlers out of `production_verified` runtime bindings. This is not a durable distributed broker or broad shell authority. |
| Production-shaped gateway runtime config/readiness | `server/src/config.rs`, `server/src/bin/secs-gateway.rs`, `server/tests/{runtime_config,readiness}.rs`, `scripts/production-gateway-smoke.sh` | Solid / implemented as local runtime hardening | Canonical gateway startup reads typed runtime config, `production_verified` requires explicit non-fixture receiver audience, verifier key path, trust-registry path, limits, bind address, and DB URL before serving; readiness can distinguish config-loaded, ledger-ready, and trust-registry-ready/fixture-only state; the smoke script runs fixture-only local checks without real secrets. This is not deployed production and does not complete trusted issuer/root policy. |
...
| Manifest-to-execution wiring | The gateway creates a signed context from descriptor lookup and handler selection uses the descriptor `handler_id`, with production runtime bindings withholding dev subprocess handlers by default. | “Descriptor-bound local execution broker,” not a durable distributed broker or arbitrary shell surface. |
...
| Trusted issuer/root lifecycle | Own-verifier public keys have B4 status/validity checks, but static trusted issuer/root entries and federated credential status policy remain Track E work. | “Own configured verifier-key lifecycle seam,” not complete production issuer/root registry. |
...
| Event/receipt ledger | `server/src/ledger.rs` | Implemented with runtime SQL and in-memory SQLite tests; gateway/ingress write reject, verify, execution, and handler lifecycle audit records. DDL is centralized in `server/src/schema.rs` rather than embedded inline in ledger/gateway runtime methods. |
...
| Bounded execution broker accepting verified context | `server/src/gateway.rs`; future `server/src/execution.rs` | Solid / implemented as local bounded broker: handlers consume `VerifiedCallContext`, are selected by descriptor `handler_id`, enforce payload/output/timeout limits, emit execution receipts for every handler outcome, and do not expose dev subprocess handlers in `production_verified` bindings. Future work may extract this into a durable/general `execution.rs` module. |
