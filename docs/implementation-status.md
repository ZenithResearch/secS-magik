...
| Identity/signature helpers for contexts/receipts | `server/src/identity.rs` | Implemented through B2: production identity loading requires an operator-provided key path, default `signer_key_id` is derived as `ed25519:<sha256-public-key-fingerprint>`, safe explicit overrides reject path/secret-shaped ids, contexts/receipts sign with the configured identity, and `PublicVerifierKeyRegistry` provides the first local public-key lookup seam for unknown-id and wrong-key rejection. |
