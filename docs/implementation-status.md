...
| Node/verifier identity key config | `server/src/identity.rs`, `server/tests/identity.rs` | Solid / implemented for explicit local file config | `production_verified` requires an explicit verifier key path before signing; missing, inaccessible, and malformed files fail as typed identity config errors; tests use only the explicit fixture helper for local/dev identities. |
...
| Identity/signature helpers for contexts/receipts | `server/src/identity.rs` | Implemented for B1 explicit verifier key config: production identity loading requires an operator-provided key path, exposes a signer key id/public key, signs contexts/receipts through the configured identity, and keeps local/dev fixture identities visibly non-authoritative. |
