...
This spec defines the M15 `dregg_authority` semantics gate. It separates M12.3 Dregg-shaped evidence, M14 `dregg_backed`, and the bounded M15 production-shaped authority seam while preserving receiver-held root/trust data, epoch-scoped root policy, and freshness/revocation/finality/non-amplification posture. M15.2–M15.8 now implement static receiver-held registry, policy-admission, descriptor-composition, local-operator disclosure, receiver-held resource-lock evidence, attenuation, and bounded local demo reconciliation; #159 is fail-closed proof/finality blocker posture, #162 wires bounded live TCP evidence propagation into the canonical evidence adapter path, and #206 is implemented at the no-live-network source-client boundary. These are bounded seams, not live Dregg authority/finality/proof rails.
...
This is no live Dregg revocation proof, no BLS threshold finality, and no rotated-replay proof verification. #162 live ingress evidence refs do not change that blocker posture; #144/M15.8 reconciles only the bounded local M15.8 resource-lock / attenuation demo boundary while keeping live verifier paths as non-goals for this repository; live Dregg revocation proof, BLS threshold finality, and rotated-replay proof verification remain unsupported. The live Castalia Dregg authority source/client (#206) contract is specified separately in `docs/specs/dregg-live-source-client-contract.md`; it is implemented at the no-live-network source-client boundary with injected transport/cache/readiness, not as live outbound Castalia/Dregg service operation.
...
This issue does not close broad live Dregg authority. #159 is resolved as fail-closed blocker posture for live Dregg revocation proof, BLS finality, and rotated-replay proof verification: missing live dependencies reject rather than silently accepting fixture/configured material as proof. #160 implements receiver-held resource-lock evidence; #141 binds operation/resource through receiver-local descriptor and request policy only, not live Dregg resource-lock authority.
...
This issue does not implement #159 live Dregg revocation proof, BLS finality, or rotated-replay proof verification. #162 live TCP evidence-ref/public-input propagation has since landed as bounded carrier plumbing, but it does not change this authority/finality boundary. It is not deployment proof (#33), public auditability (#37), Midnight (#74), Cardano (#75), or broad live Dregg authority closure.
...
#144/M15.8 reconciles the bounded M15.8 resource-lock / attenuation boundary across #162 live ingress evidence refs/public inputs, #167 delegated attenuation / non-amplification, #169 trusted requested-authority attenuation, and #160 implements bounded receiver-held resource-lock evidence. The finalizer preserves `resource_lock:verified` acceptance, `resource_lock_violation` rejection, redaction-safe operator summaries, and signed-context propagation of the verified locked resource for handler/policy use. See `examples/m15-dregg-authority-demo.sh` for the bounded production-shaped demo/checklist. This is not deployment proof, not public auditability, not live Dregg revocation proof, not BLS threshold finality, not rotated-replay proof verification, not Midnight, and not Cardano.

## I07 authority-mode labels

I07 authority-mode labels are implemented as a bounded verifier/policy label surface: `local_fixture`, `signed_source`, `solo_verified_receipt`, `federation_checkpoint`, `light_client_verified`, and `recursive_proof_carrying_state`. Policy comparison rejects weaker or unknown modes with `authority_mode_downgrade` / reserved-mode reasons before handler execution and emits redacted observed/required authority-mode summaries. signed_source in this slice is a label/fixture policy tier only; live signed-source transport remains gated by I16. federation_checkpoint in this slice does not claim durable rollback/equivocation-resistant finality; durable federated checkpoint/root state remains gated by I17. light_client_verified and recursive_proof_carrying_state are reserved fail-closed labels until their verifier issues land in I18 and I19. Node/listing/downstream federation topology such as delegated-under-node metadata is not federation-checkpoint evidence and does not authorize operations by itself.
