# secS-magik phase issue specs

This directory contains repo-local, issue-ready phase specs for remaining secS-magik ready-for-production tracks. Phase GitHub issues are the tracking containers; tasks inside each phase issue are intended commit boundaries.

## Active specs

| Track | GitHub issue | Local spec | Status |
|---|---:|---|---|
| Track E — trusted issuer/root policy | #63 / underlying #35 | [`track-e-trusted-issuer-root-policy.md`](track-e-trusted-issuer-root-policy.md) | locally implemented through E11; E12 PR/CI/merge gate pending |

## Boundary

These specs are planning/control artifacts. Implementation still happens on a focused phase branch and must update `README.md`, `server/README.md`, `docs/implementation-status.md`, `docs/repository-schema.md`, `docs/plans/2026-06-02-ready-for-prod-checklist.md`, `CHANGELOG.md`, the vault master checklist, and the relevant GitHub issues/PR body when a phase lands.
