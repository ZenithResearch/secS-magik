...
fn live_dregg_source_client_contract_is_defined_before_runtime_implementation() {
    for required in [
        ...
        "persistent-cache helper slices",
        "DreggLiveSourceFileCache",
        "stale persistent cache",
        "fail closed",
...
fn authority_mode_docs_record_i07_labels_without_stronger_rail_overclaims() {
    ...
    for required in [
        ...
        "descriptors that set receiver-held `required_authority_mode`",
        "Verified adapter output supplies only the observed `authority_mode`",
        "`ConfigurableRouter::route_verified` compares the observed mode against the active descriptor",
        "before replay/nullifier use-state, verify/success receipts, handler lookup, or handler dispatch",
        "executes no registered handler",
